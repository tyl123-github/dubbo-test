package com.tyl.rabbitmq.config;
...
public class RabbitMQConfig {
    ...
    public static final String DEAD_LETTER_QUEUEA_ROUTING_KEY = "delay.queue.demo.deadletter.delay_15m.routingkey";
    public static final String DEAD_LETTER_QUEUEB_ROUTING_KEY = "delay.queue.demo.deadletter.delay_20m.routingkey";
    ...
    public Queue delayQueueA(){
        ...
        args.put("x-message-ttl", 1000*60*15);
    ...
    // 声明死信队列A 用于接收延时15m处理的消息
    @Bean("deadLetterQueueA")
    public Queue deadLetterQueueA(){
        return new Queue(DEAD_LETTER_QUEUEA_NAME);
    }

    // 声明死信队列B 用于接收延时20m处理的消息
    @Bean("deadLetterQueueB")
    public Queue deadLetterQueueB(){
        return new Queue(DEAD_LETTER_QUEUEB_NAME);
    }
    // 声明死信队列c 用于接收延时10s处理的消息
